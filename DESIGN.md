...
Designbeslut för Spark. **"Gällande designsystem" nedan är det som gäller.**
"Historik" längre ner är tidigare sessioners beslut, oförändrade. Mycket där
beskriver `components/spark` och det gamla demot, som fasas ut.

## Gällande designsystem

Från PR 1 i `docs/plan-en-design.md`, som teamet har godkänt.

### Besluten
1. **Demots stil är appens stil.** Startsidan, demot (`/demo`) och den
...
### Var stilen finns
- **`design/site.css`**, laddad av `app/(marketing)/layout.tsx`,
  `app/demo/layout.tsx` och `app/(app)/layout.tsx`. Allt är skopat:
  - `.fd` är grunden: typografi, knappar (`fd-btn`), etiketter, paneler
    (`fd-panel`), sidhuvud och sidfot, och startsidans sektioner.
...
- **Typsnitt**: Castoro (`--font-sans`) för brödtext och rubriker,
  Instrument Serif Italic (`--font-serif-italic`) bara för betoningsord.
  Självhostade via `design/fonts.ts`.
- **Rörelse**: bara `transform`, `opacity` och `clip-path`. Med
  `prefers-reduced-motion` blir rörelsen lugnare, inte avstängd (en tvär
  växling ser ut som ett fel). Rundturens tider och kurvor står i
  `app/demo/_lib/tourMotion.ts`.

### Skärmar och data
- **Skärmarna vet aldrig varifrån datan kommer.** De tar props och typer
  från `ports/` och `core/`, aldrig från `adapters/`. Vakten
  `screens/noAdapters.guard.test.ts` blir röd annars.
- **Datalöftet i gränssnittet**: källa på varje siffra (`SourceTag`),
  simuleringar märkta "Simulering", Hiasynth och Lovable med
  `ConceptBadge`. Påhittad data märks synligt med `ExampleLabel`
...
Tidigare sessioners designbeslut, i den ordning de togs.
...
### Formgivningspass mot artefakten, fullständigt (gren `prototyp`)

Uppdrag: artefakten är specifikationen överallt där vår kod och artefakten skiljer sig — tidigare sessioners medvetna avsteg ska bort, inte försvaras. Fyra uppgifter: (1) typografin, (2) skalet, (3) de sex sidor som har en artefaktmotsvarighet, sektion för sektion, (4) de fyra sidor som inte har det (Resan, Poäng, Pulsen, Juridik), byggda av samma byggstenar. `core/score.ts`, `adapters/live/`, `lib/server/`, `ports/` och demodatan orörda. Tio commits.

#### Uppgift 1 — Typografin
- **`--font-data` borttaget helt** (`design/fonts.ts`s `dataSans`-export, `app/layout.tsx`s klassträng, `design/fonts/funnel-display/` raderad). Artefaktens `--mono` är samma typsnitt som `--sans` (Castoro) — siffror har ingen egen font. `.font-numeric` (`app/globals.css`) omdefinierad till bara `letter-spacing:-0.005em` (artefaktens `.num`), ingen `font-family`-rad längre.
- **`font-bold`/`font-extrabold` borttaget från alla rubriker** (grep bekräftade tolv träffar: `EditorialHeading`, `ScoreBadge`, `NextStepCard`, `JourneyRail`, `SimulationCard`, `TourOverlay`, `Journey`, `Memory`, `OnboardingEntry`, `OnboardingIdea`, båda marketingsidorna) — Castoro finns bara i vikt 400, fetstilen renderades som webbläsarens syntetiska variant. Hierarkin byggs om med storlek i stället, efter artefaktens egen dokumenterade kompensationstabell (`app.css` rad 611–674, en kommentar skriven uttryckligen för att Castoro bara har en vikt): `EditorialHeading`s standardstorlek `text-4xl`→`text-3xl` (matchar pagehead h1:s 29px), och åtta enskilda rubrikställen fick sin storlek höjd ett Tailwind-steg när `font-bold`/`-extrabold` togs bort (t.ex. `NextStepCard`s titel `text-2xl`→`text-3xl`, matchar `.act h2.big`s 27px).
- **`ScorePanel`s stora poängtal fick tillbaka `.font-numeric`** — det tidigare "grundarens uttryckliga undantag" (se ovan, "Poängvisningen") handlade om att INTE peka ut Funnel Display; nu när `.font-numeric` inte längre byter typsnitt är undantaget överflödigt, klassen lades tillbaka för samma tighta spårning alla andra tal har.
- Verifierat: `pnpm typecheck`/`lint`/`test`/`build` gröna.

#### Uppgift 2 — Skalet
- **Sidomenyns brand-rad:** ett litet ikonmärke (blixt-SVG i en `bg-white/10`-ruta, artefaktens `.brand .mk` men ljus i stället för navy eftersom sidomenyn själv är mörk) + `Logo` + en ny undertext `t.appShell.tagline` ("AI-medgrundare"/"AI co-founder").
- **Navposterna fick ikoner** — ny `components/spark/NavIcon.tsx`, tio egna linjeikoner (24×24, stroke 1.9) i artefaktens visuella språk, inte kopierade ur dess `ICONS`-register (som aldrig lästes — bara konceptet "en ikon per post" återanvändes). **Räknare:** bara Hem fick en (`upplåsta/totalt delar`, härlett ur redan hämtad `scoreSnapshot` — motsvarar artefaktens `kritKlara()/4`). Ingen räknare på Validering (artefaktens `S.svar`-motsvarighet skulle krävt en ny datahämtning i skalet för en decorativ siffra — avstod, flaggat nedan).
- **Ny sidfot** (`.side-foot`): profilavatar+namn (flyttat från sidhuvudet) och en ny `SidebarRestart`-knapp (demo-bara, samma `useDemoStore().reset()`-anrop som demoradens "Återställ" — en andra ingång till samma åtgärd, inte en duplicerad implementation). En tidigare sessions "rör inte sidstruktur"-avgränsning gäller inte den här uppgiften, som uttryckligen bad om just den här flytten.
- **Sidhuvudet** delar nu brödsmula och stegpill på två rader (artefaktens `.crumb`/`.stepnow`) i stället för en sammanslagen textrad.
- `aside` fick `sticky top-0 h-screen` (för att sidfoten faktiskt ska synas i viewporten, inte längst ner på en scrollbar sida) med `pb-20` när demoraden (`bottomBar`) är monterad, så sidfoten inte hamnar bakom den fasta demoraden.
- **Grundarens uttryckliga undantag kvarstår:** sidomenyn är `--navy`, inte artefaktens ljusa `color-mix`.
- Verifierat: `pnpm typecheck`/`lint`/`test`/`build` gröna, klickat igenom med Playwright på flera sidor (skärmdumpar), inga konsolfel.

#### Uppgift 3 — Sidorna (en commit per sida)
- **Hem:** ingen pagehead ovanför hero-griden — artefaktens `vyHem` har ingen, handlingskortets egen rubrik bär vikten (den gamla "Idag · [datum] / Ditt nästa steg."-rubriken togs bort). maxw 1200px→1080px, hero-griden till en fast 316px-sidospalt och 18px-gap. **`SuggestionList` omskriven** från en radlista till artefaktens `.sugggrid`-mönster (rutnät av kort: poäng+titel överst, tidsuppskattning + lucktyp + knapp i foten) — ny i18n `scorePage.estimatedMinutesUnit`/`suggestionsSortNote`. **Den dubbla kantlinjen** (grundarens uttryckligen flaggade kända avsteg) fixad genom en delad rutnätslinje (`gap-px` + bakgrundsfärg som lyser igenom) i stället för att varje kort bar egen kant+skugga ovanpå `Card`-omslaget — samma fix på `PulseCard` (ny `bare`-prop). Fyllnadsceller (osynliga, bara ≥sm) täcker en ofullständig sista rad så den inte syns som en hel grå ruta.
- **Medgrundaren:** pagehead statisk igen ("Medgrundaren" + generell beskrivning) i stället för att bytas ut mot det aktuella momentets etikett — den hör hemma på chattkortets egen rubrik (oförändrad sedan tidigare). maxw och `.cog`-griden till samma 1080px/316px/18px-mönster.
- **Marknad:** "Storleksfördelningen" och "Konkurrenterna" var båda bara `Eyebrow` + bar div/grid — artefaktens `dist`/`konk` går genom `card(chead(...), ...)`, båda nu `Card`-wrappade (Storleksfördelningen med en "SNI"-not till höger). Konkurrenterna använder samma rutnätslinje som Hems `SuggestionList`. `.split`-griden till 316px/18px.
- **Validering:** "Antagandena som prövades" hade samma dubbla-kantlinje-bugg som Hem (varje antagande sin egen kort-yta inne i en `Card`) — fixad till rader med en avdelarlinje (artefaktens `.assum li`), ingen egen kant/skugga per rad.
- **Bygg:** `.buildwrap`s sidospalt är en fast 300px i artefakten, inte en `1fr`-andel — fixat. "Omfånget"-kortet fick sin "steg 08"-not (ny i18n `buildPage.scopeStepNote`).
- **Minnet:** pagehead till artefaktens ordning — "Namn, ålder, ort" som rubrik, bion som ingress (samma två datafält, `profile.name`/`role`, som redan fanns, bara omvända — tidigare var bion h1:en och namn/roll en Eyebrow ovanför). `profgrid`-gap 16px→18px, Hjärnans textarea `rounded-lg`→`rounded-md`.
- Verifierat vid varje commit: `pnpm typecheck`/`lint`/`test`/`build` gröna, skärmdumpar via Playwright av alla sex sidorna.

#### Uppgift 4 — Resan, Poäng, Pulsen, Juridik
Genomgång visade att en tidigare session ("Sidornas komposition") redan hade byggt dessa fyra med samma komponenter/skala som de sex artefaktsidorna — bara två kvarvarande avvikelser: Resans fasgap 24px→18px, och Poängs "Höj din poäng"-kort saknade sorteringsnoten Hem redan hade. Pulsen och Juridik hade inget att rätta. Ingen av de fyra byggdes om i grunden — de ärver `SuggestionList`/`PulseCard`s dubbla-kantlinje-fix och Uppgift 1:s typografi automatiskt, eftersom de redan återanvände samma delade komponenter.

#### Vad som INTE kunde matchas, och exakt varför
1. **"Kundlistan" (kontaktstatus per byrå) lever på Validering, inte Marknad.** Artefaktens `vyMarknaden` avslutas med en kontaktstatustabell — hos oss slogs "Kunder" och "Valideringen" ihop till en sida i en session före den här (`i18n/dictionary.ts:305`s kommentar). Att flytta hela sektionen (med sin egen tabell, sina egna tour-steg, sin egen testtäckning) är en sidoombyggnad, inte en sektionsjustering inom en sida — utanför den här sessionens ram, flaggat i Marknad-commiten.
2. **Validerings-räknaren i sidomenyn** (artefaktens `S.svar`-siffra på navposten) byggdes inte — den datan (antal svar) är inte del av det AppShell redan hämtar (`profile`/`scoreSnapshot`/`currentStep`), och att lägga till en ny datahämtning i det delade skalet bara för en decorativ räknare bedömdes fel avvägning. Hem fick sin räknare eftersom den var gratis (redan i `scoreSnapshot`).
3. **`VerdictCard` byggdes inte om** till artefaktens rikare `.verdict`-kort (egen färgad rubrikrad "Domen · steg N", en `.ac`-åtgärdsruta, ett separat detaljstycke). Komponenten delas av tre ställen (Validering, JourneyStep steg 06, landningssidan) — en full ombyggnad är en egen, större uppgift, inte en sektionsjustering, och ingen founder-flaggad "känd avsteg" pekade på just den.
4. **Byggets "grindrad"** (artefaktens `gatebar`, en egen rubrik+beskrivning om huruvida omfånget vilar på ett låst underlag) visar hos oss bara byggstatus — den datan (är steg 06 låst) finns inte i `BuildData` i dag.
5. **Tabellens interna typografi** (mono/versal i `th`) och **mobilanpassningen** (`.side{display:none}` + horisontell pill-meny) — samma två punkter som en tidigare sessions `DESIGN.md`-post redan flaggade som utanför ramen, oförändrat.
6. **Minnets "Härifrån kom idén"-kedjekort** — ingen strukturerad data för kedjan finns, samma godkända avgränsning som en tidigare session redan dokumenterade.

#### Manuell webbläsarverifiering
Playwright (cachad `npx`-installation, `~/.npm/_npx/*/node_modules/playwright`, Chromium i `~/.cache/ms-playwright`) mot `pnpm build && pnpm start`. Klickat igenom alla nio `/demo/app`-sidorna på **både sv och en** (nav-länkar, inte `page.goto` till nästa route — se kända problem nedan), skärmdumpar av Hem/Medgrundaren/Marknad/Validering/Bygg/Minnet/Resan/Poäng/Pulsen/Juridik. Inga konsolfel i någon av de 18 kombinationerna.

#### Kända problem / upptäckt under sessionen, inte skapat av den
**`/demo/app/<undersida>` kan redirecta till `/demo/start` vid en FRESH full sidladdning**, trots att `onboardingDone:true` redan är persisterat korrekt i `localStorage` (bekräftat med en direkt localStorage-inspektion). Reproducerat med `page.goto("/demo/app/marknad")` direkt (utan att först ha varit inne i appen i samma sida-session) — en Zustand-hydreringskapplöpning i `app/demo/app/layout.tsx`s `if (!onboardingDone) router.replace(...)`-effekt, som tycks läsa butikens state innan `persist`-mellanlagret hunnit hydrera från localStorage vid just den här typen av navigering. Reproducerar INTE vid vanlig SPA-navigering (klick på en `Link` i sidomenyn, det här sessionens hela verifieringsmetod) eller vid navigering till `/demo/app` själv. Inte undersökt vidare eller fixat — utanför den här sessionens uppgift (typografi/skal/sidor), men flaggat tydligt eftersom det skulle förvirra en presentatör som laddar om sidan mitt i en undersida. En framtida session bör undersöka `app/demo/app/layout.tsx`s onboarding-redirect-effekt specifikt vid en hård sidladdning till en nästlad route.
