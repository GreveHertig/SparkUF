...
### Mejlfältet kopplat till väntelistan (2026-09-26)
- **Väntelistans databas:** migreringen `20260924120000_waitlist.sql` är **redan körd mot SparkUF2 (2026-09-26)** och verifierad av Erik: RLS på, inga policyer, anon kan varken göra insert eller select på `public.waitlist`, och anon kan köra `join_waitlist`. (Status på `prototyp` säger fortfarande att den inte är körd; det stämmer inte längre.) **Kör inte `supabase db push`**: den skulle också köra `registry_cache`, som väntar på §6 fråga 4.
- **Oskars kod återanvänd oförändrad:** `app/(marketing)/actions.ts` (`joinWaitlist`) och `actions.test.ts` är hämtade från `origin/prototyp` med `git checkout`, byte för byte, så de krockar inte när grenen flyttas till `prototyp`. Migreringsfilen är inte med på grenen, och tabellen är orörd.
- **`app/experiment/fonda/actions.ts`** (`joinFondaWaitlist`): ett tunt lager framför `joinWaitlist`. Honeypot-fältet `website` (`_lib/waitlist.ts`): ifyllt ger samma "klart" som för en människa men inget sparas. Spärr per IP (första adressen i `x-forwarded-for`, annars `x-real-ip`) via `checkRateLimit` i `lib/server/rateLimit.ts`: 5 per timme och 20 per dygn. Returnerar bara koder (`joined`, `email_invalid`, `unexpected`, `rate_limited`), ingen text.
- **`_components/EmailSignup.tsx`:** `useActionState`. Validering i webbläsaren innan något skickas (fel under fältet, `aria-invalid`), knappen inaktiverad med "Skriver upp dig …" medan det skickas, tacket "Tack! Vi hör av oss när betan öppnar." i stället för formuläret, fel som text. Samma tack oavsett om adressen redan fanns. `<label>`, `type="email"`, `autocomplete="email"`, meddelanden i en `role="status"`/`aria-live="polite"` som alltid är monterad. Knappen heter "Skriv upp mig".
- **Integritet:** en rad under fältet (varför vi sparar adressen, att man kan be om att bli borttagen) med länk till den nya sidan `/experiment/fonda/integritet`: vad som sparas (adressen och tidpunkten), vem som är personuppgiftsansvarig (Spark UF), varför (med samtycke, för att meddela när betan öppnar), var (Sparks databas hos Supabase, inom EU, bara Spark UF-teamet ser listan), hur länge (raderas efter lanseringen eller tidigare på begäran) och hur man blir borttagen (mejl till spark.ai.uf@gmail.com, samma adress som Oskars text). "Inom EU" är kontrollerat: SparkUF2 ligger i `eu-central-1` (Frankfurt) enligt `supabase projects list` och pooler-värden `aws-0-eu-central-1`. i18n i sv och en (`experimentFonda.close`, `experimentFonda.privacy`); de gamla texterna "inte kopplat, inget sparas" är borta.
...
### Mejlfältet: MX-kontroll och "Menade du …?" (2026-09-26)
- **MX-kontroll i Server Action** (`_lib/mxCheck.ts`, anropas i `actions.ts` efter honeypot och IP-spärr, före Oskars `joinWaitlist`): `resolveMx` från `node:dns/promises` på domänen. `ENOTFOUND`, `ENODATA`, ett tomt svar eller en null MX (RFC 7505, t.ex. `example.com`) ger koden `email_undeliverable` och texten "Den adressen verkar inte kunna ta emot mejl. Kolla stavningen." Tar uppslaget mer än 3 s, eller ger ett annat fel, släpps adressen igenom. Adresser utan domän slås inte upp; `joinWaitlist` avvisar dem som förut.
- Medvetet val: en domän utan MX men med en A-post kan enligt RFC 5321 ta emot mejl ändå ("implicit MX"). Den stoppas nu (`ENODATA`), enligt beställningen. Sådana domäner är ovanliga för personliga adresser.
- **"Menade du …?"** (`_lib/emailTypos.ts`): klickbar rad under fältet för vanliga stavfel i domänen (gmial/gmal/gamil/gmai/gnail → gmail.com, hotmial/hotmal → hotmail.com, outlok/otlook → outlook.com, iclod/icoud/icluod/iclould → icloud.com, och `.con`/`.cm`/`.cmo` på de fyra). `hotmail.se` och `outlook.se` lämnas i fred. Klick byter adressen i fältet; utan klick skickas adressen som den står.
- Tester (DNS mockad, inget nätverk): domän utan MX stoppas (`ENOTFOUND`, `ENODATA`, null MX), gmail.com går igenom, timeout efter 3 s och andra DNS-fel släpps igenom, ingen uppslagning för adress utan domän; `emailTypos.test.ts`; formuläret visar förslaget, klick rättar, inskicket stoppas inte, MX-felet visas.
- Kontrollerat med riktig DNS från containern: gmail.com har MX, en påhittad domän ger `ENOTFOUND`, example.com har null MX, alla under 100 ms.
- Oskars filer i `app/(marketing)/` är orörda. Ingen migrering.
...
### Återstår
- Inget i kopian.
- Väntelistan: prova ett riktigt inskick mot SparkUF2 med en egen adress (och ta bort raden i dashboarden efteråt). Integritetstexten bör läsas av grundarna innan sidan visas publikt, särskilt löftena "delar den inte med någon" och "raderas efter lanseringen", som kräver att någon faktiskt tömmer listan.
...
- Väntelistans spärr per IP ligger i minnet: den nollställs vid omstart och delas inte mellan serverinstanser (samma begränsning som `rateLimit.ts` redan har). Utan proxy som sätter `x-forwarded-for` delar alla besökare nyckeln `unknown`. Headern går att förfalska om proxyn framför Next inte skriver över den (Vercel gör det), och då går spärren att kringgå. Honeypoten och `join_waitlist` (samma svar, inga läsrättigheter) gäller ändå.
