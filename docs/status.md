...
- **Montering:**
  - `/demo/app` (`app/demo/app/layout.tsx`, `page.tsx`, `loading.tsx`): demoadaptrarna, anropas direkt från klienten med `use()` (memoiserat per `locale` via `useMemo` — se `docs/arkitektur.md` avsnitt 7).
  - `/app` (`app/(app)/layout.tsx`, `app/(app)/app/page.tsx`): liveadaptrarna, async Server Components. Sidhuvudet visar en neutral platshållare (`—`/`—`, ingen poängbricka) om profil/poäng inte är byggda; själva sidan visar `<ComingSoon />`.
...
- **`use()` + `useMemo`-mönstret** i `app/demo/app/*` måste följas av framtida demoroutes (Session 3 m.fl.) — en promise som skapas på nytt vid varje rendering utan `useMemo` är en känd anti-pattern för `use()`.
