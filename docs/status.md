...
- **Alla sidor** finns i `/experiment/fri`s stil: onboardingen (`start`, `start/profil`, `start/ide`), Hem, Medgrundaren, Resan med stegsidor, Poäng, Marknad, Validering, Pulsen, Minnet, Juridik, Bygg och Affärsplanen, samt demoraden (alla funktioner och genvägar ← → T R) och rundturen (samma 20 stopp, rutterna översatta till kopian). "Se demot" på `/experiment/fri` går hit.
- **Datan kommer ur de riktiga demoadaptrarna, oförändrade.** Kopian växlar demo-lagrets lagringsnyckel till `spark:fri-demo-state` medan man är i den, med zustands publika `persist.setOptions`/`rehydrate`, och växlar tillbaka när man lämnar (`_lib/friDemoIsolation.ts`). Det riktiga demots sparade läge (`spark:demo-state`) läses och skrivs aldrig av kopian. `friDemoIsolation.test.ts` bevisar det, och att poängen blir densamma. Det tidigare speglade datalagret och den kopierade pulsdatan är borttagna.
- **Skillnader i beteende mot originalet (medvetna):** onboarding-spärren läser lagrets faktiska värde, så att en omladdning inte skickar en återvändande besökare till onboardingen. Det riktiga demot har den buggen: hooken ger serverns utgångsläge under hydreringen. Knapparna "Öppna steget" och stegkorten på Resan länkar inom kopian.
- Verifierat att `/demo` är orört: inga filer utanför `app/experiment/fri` och `i18n` ändrade, och skärmdumpen av `/demo/app` är byte-identisk med den som togs före arbetet.
