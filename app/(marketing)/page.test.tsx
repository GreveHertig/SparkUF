...
vi.mock("./landingActions", () => ({ joinLandingWaitlist: joinMock }));
...
describe("/", () => {
  ...
  it("växlar till engelska", async () => {
    ...
    expect(screen.getByRole("heading", { level: 1 })).toHaveTextContent(en.site.hero.titleEm);
    expect(screen.getByText(en.site.price.amount)).toBeInTheDocument();
  });

  it("nämner inte Fonda, påstår inget om att vara ensam och har inga tankstreck", () => {
    for (const text of [...allStrings(sv.site), ...allStrings(en.site)]) {
