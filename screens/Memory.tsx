  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-8">
      ...
      <Tabs.Root defaultValue="profile">
        ...
        <Tabs.Content value="profile" className="pt-6">
          <div className="rounded-lg border border-slate-200 bg-white p-6">
            ...
            <div className="mt-4">
              ...
              <p className="mt-1 text-sm text-slate-700">{data.profile.bio}</p>
            </div>
            <div className="mt-4">
        ...
        <Tabs.Content value="brain" className="pt-6">
          ...
          <textarea
            ...
            rows={6}
            className="w-full rounded-lg border border-slate-200 bg-white p-4 text-sm text-slate-900 focus-visible:outline-2 focus-visible:outline-accent-300"
          />
        </Tabs.Content>

        <Tabs.Content value="trace" className="pt-6">
          {data.trace.length === 0 ? (
            <p className="text-sm text-slate-600">{t.memoryPage.traceEmpty}</p>
          ) : (
            <ol className="flex flex-col gap-3">
              {data.trace.map((event) => (
                <li key={event.id} className="flex gap-4 rounded-lg border border-slate-200 bg-white p-4">
                  <span className="w-24 shrink-0 text-xs text-slate-500">{formatDate(event.timestampIso, locale)}</span>
                  <span className="text-sm text-slate-800">{event.description}</span>
                </li>
              ))}
            </ol>
          )}
